...
async def view_created(call: CallbackQuery):
    ...
    created = [r for r in reminders if r.author.telegram_id == call.from_user.id]
...
async def view_assigned(call: CallbackQuery):
    ...
    assigned = [
        ...
        if any(rec.user.telegram_id == call.from_user.id for rec in r.recipients)
    ]
...
async def view_reminder(call: CallbackQuery):
    ...
    recipients = ", ".join([rec.user.name for rec in reminder.recipients]) or "нет"
