...
async def step_recipients(msg: Message, state: FSMContext):
    await state.update_data(text=msg.text, recipients=[])
...
# ============================
@router.callback_query(ReminderStates.recipients, F.data == "recipients_done")
async def step_recipients_done(call: CallbackQuery, state: FSMContext):
...
async def finish(call: CallbackQuery, state: FSMContext):
    ...
    author_name = USERS.get(author_id, call.from_user.full_name)

    rec_links = []
    for uid in recipients:
        name = USERS.get(uid, "Пользователь")
        rec_links.append(f'<a href="tg://user?id={uid}">{name}</a>')
    rec_html = ", ".join(rec_links) if rec_links else "нет"
...
# ============================
# КНОПКА «Мои напоминания» (плоский список с удалением)
# ============================
@router.callback_query(F.data == "my_reminders")
async def my_reminders(call: CallbackQuery):
    reminders = await service.get_user_reminders(call.from_user.id)

    if not reminders:
        await call.message.answer("У тебя пока нет напоминаний 🙂", reply_markup=main_menu())
        return

    await call.message.answer("📋 Твои напоминания:")

    for r in reminders:
        dt = r.target_datetime.strftime("%Y-%m-%d %H:%M")
        ...
        if r.recipients:
            ...
            for rec in r.recipients:
                ...
                name = rec.user.name
                rec_links.append(f'<a href="tg://user?id={uid}">{name}</a>')
            rec_names = ", ".join(rec_links)
        else:
            rec_names = "нет получателей"

        text = (
            f"🔔 <b>Напоминание</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"📝 <b>Текст:</b> {r.text}\n"
            f"📅 <b>Дата:</b> {dt.split()[0]}\n"
            f"⏰ <b>Время:</b> {dt.split()[1]}\n"
            f"👤 <b>Автор:</b> {r.author.name}\n"
            f"👥 <b>Получатели:</b> {rec_names}\n"
            f"⏳ <b>За:</b> {r.notify_before_minutes} минут\n"
            f"🆔 <b>ID:</b> {r.id}\n"
            f"━━━━━━━━━━━━━━━━━━"
        )
        ...
        if r.author.telegram_id == call.from_user.id:
            kb_buttons.append([InlineKeyboardButton(text="❌ Удалить", callback_data=f"delete_{r.id}")])
...
async def delete_confirm(call: CallbackQuery):
    ...
    if not reminder or reminder.author.telegram_id != call.from_user.id:
        await call.answer("Можно удалять только свои напоминания.", show_alert=True)
        return

    text = f"Удалить напоминание?\n\nID: {reminder_id}"
...
# УДАЛЕНИЕ — выполнение
# ============================
@router.callback_query(F.data.regexp(r"^delete_yes_\d+$"))
async def delete_yes(call: CallbackQuery):
    ...
    await call.message.edit_text("Напоминание удалено ✔️", reply_markup=main_menu())
...
# ============================
# Подтверждение получения напоминания
# ============================
@router.callback_query(F.data.regexp(r"^confirm_reminder_\d+$"))
async def confirm_reminder(call: CallbackQuery):
    reminder_id = int(call.data.split("_")[2])

    receiver_id = call.from_user.id
    receiver_name = USERS.get(receiver_id, call.from_user.full_name)

    rec = await service.confirm_reminder(reminder_id, receiver_id)
    reminder = await service.get_reminder(reminder_id)
    ...
    if rec is None:
        await call.answer("Ты не назначен получателем этого напоминания.", show_alert=True)
        return

    author_telegram_id = reminder.author.telegram_id

    text_for_author = (
        f"✔️ <b>{receiver_name}</b> подтвердил(а) напоминание!\n"
        f"━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        await call.bot.send_message(author_telegram_id, text_for_author, parse_mode="HTML")
    except Exception as e:
        print(f"Ошибка отправки автору: {e}")

    await call.message.edit_text("Спасибо! Напоминание подтверждено ✔️")
